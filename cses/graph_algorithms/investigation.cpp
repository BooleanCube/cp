...
void solve() {
  ...
  vector<array<int, 4>> dist(n, {INF, 0, INF, -INF}); // min price, num of paths, min flights, max flights
  priority_queue<array<int, 3>, vector<array<int, 3>>, greater<array<int, 3>>> pq; pq.push({0, 0, 0});
  vi vis(n);
  dist[0] = {0, 1, 0, 0};
  while(!pq.empty()) {
    auto [p, f, u] = pq.top(); pq.pop();
    if(vis[u]) continue; vis[u] = 1;
    for(auto [v, w] : adj[u]) {
      int np = dist[u][0] + w;
      if(dist[v][0] < np) continue;
      else if(dist[v][0] == np) {
        (dist[v][1] += dist[u][1]) %= MOD;
        dist[v][2] = min(dist[v][2], dist[u][2]+1);
        dist[v][3] = max(dist[v][3], dist[u][3]+1);
      }
      else dist[v] = {np, dist[u][1], dist[u][2]+1, dist[u][3]+1};
      pq.push({np, f+1, v});
    }
  }
  printv(dist[n-1]);
